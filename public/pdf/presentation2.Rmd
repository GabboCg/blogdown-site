...
Existen más opciones, pero no serán necesarias para la sesión. Con la función `p_load` agregan^[no va a funcionar si no escriben antes `if(!require("pacman")) install.packages("pacman")` ] todos las librerías que deseen, si las tienen las carga (`library()`), si no, las instala (`install.packages()`) y luego las carga.
...
g2 <- g2 + ggtitle("Trade-off Riesgo-Retorno", subtitle = "Tres activos riesgosos & minima varianza")
g2
```
