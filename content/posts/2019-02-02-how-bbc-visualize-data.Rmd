...
imdb_tbl_1 <- imdb_tbl %>% 
  ...
  filter(row_number() == 1) %>% 
  ungroup() %>% 
  mutate(percent = n/sum(n), season = paste0("S", number_season)) %>% 
...
imdb_x_files_tbl %>% 
  ggplot(aes(x = season, y = av_rating)) +  geom_bar(colour = "black",
                                                     stat = "identity",
                                                     position = "identity",
                                                     fill = "orange") + 
  bbc_style() + geom_hline(yintercept = 0, size = 1, colour="#333333") +
  geom_label(aes(x = 9, y = 6.5, label = "Mulder leaves\nthe X-Files"), 
             hjust = 0, 
             vjust = 0.5, 
             lineheight = 0.8,
             colour = "#555555", 
             ...
             family = "Helvetica", 
             size = 6) +
  geom_curve(aes(x = 9, y = 6.5, xend = 8, yend = 7.9099), 
                             colour = "#555555", 
                             size=0.5, 
                             curvature = -0.2,
